    ...
    (async () => {
      ...
      if (hostname && (hostname.includes('clicksetopfile') || hostname.includes('prelander'))) {
        if (!tabAuth && isDirectNavigation) {
          console.log('[TAB-GUARD] Direct pasted URL detected - redirecting to previous URL or fallback');
          const previousUrl = document.referrer && document.referrer.length > 0 ? document.referrer : 'https://www.google.com';
