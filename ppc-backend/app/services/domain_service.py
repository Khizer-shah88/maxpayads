...
_LEGACY_KEYS = {
    "link": "platform_domain",
    "intermediate": "intermediate_domain",
    "last": "last_domain",
}
...
def _serialize(doc: dict, publisher_map: Optional[Dict[str, str]] = None) -> dict:
    ...
    return {
        ...
        "domain_type": doc.get("domain_type", "link"),
...
async def _clear_default_flag(db, domain_type: str, exclude_id: Optional[ObjectId] = None):
    query: dict = {"domain_type": domain_type, "is_default": True}
    if exclude_id:
...
async def _sync_legacy_setting(db, domain_type: str, url: str):
    ...
    key = _LEGACY_KEYS.get(domain_type)
...
async def create_domain(db, data: dict) -> dict:
    ...
    domain_type = data.get("domain_type", "link")
    ...
    existing = await db.redirection_domains.find_one({"domain": domain, "domain_type": domain_type})
    if existing:
        raise ValueError("This domain is already registered for this type")
...
async def update_domain(db, domain_id: str, data: dict) -> Optional[dict]:
    ...
    if data.get("is_default") is not None:
        ...
        if is_default:
            await _clear_default_flag(db, doc["domain_type"], exclude_id=oid)
...
async def resolve_domain_url(
    ...
    *,
    ...
    base_query: dict = {"domain_type": domain_type, "status": "active"}
    if require_verified:
    ...
    legacy_key = _LEGACY_KEYS.get(domain_type, "platform_domain")
...
async def migrate_legacy_domains(db):
    ...
    if tracking and tracking.get("value"):
        ...
        if host:
            existing = await db.redirection_domains.find_one({"domain": host, "domain_type": "link"})
            if not existing:
                await db.redirection_domains.insert_one({
                    "domain": host,
                    "domain_type": "link",
